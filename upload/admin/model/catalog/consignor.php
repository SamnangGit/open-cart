...
/**
 * Class consignee_product
 *
 * @package Opencart\Admin\Model\Catalog
 */
class Consignor extends \Opencart\System\Engine\Model
{
	/**
	 * @param array $data
	 *
	...
	public function addProduct(array $data): int
	{
		...
		//Changes: removed "', `tag` = '" . $this->db->escape($value['tag']) . "', `meta_title` = '" . $this->db->escape($value['meta_title']) . "', `meta_description` = '" . $this->db->escape($value['meta_description']) . "', `meta_keyword` = '" . $this->db->escape($value['meta_keyword']) 
		// Categories
		...
		$this->cache->delete('consignee_product');
	...
	{
		$this->db->query("UPDATE `" . DB_PREFIX . "consignee_product` SET `model` = '" . $this->db->escape((string)$data['model']) . "', `sku` = '" . $this->db->escape((string)$data['sku']) . "', `upc` = '" . $this->db->escape((string)$data['upc']) . "', `ean` = '" . $this->db->escape((string)$data['ean']) . "', `jan` = '" . $this->db->escape((string)$data['jan']) . "', `isbn` = '" . $this->db->escape((string)$data['isbn']) . "', `mpn` = '" . $this->db->escape((string)$data['mpn']) . "', `location` = '" . $this->db->escape((string)$data['location']) . "', `variant` = '" . $this->db->escape(!empty($data['variant']) ? json_encode($data['variant']) : '') . "', `override` = '" . $this->db->escape(!empty($data['override']) ? json_encode($data['override']) : '') . "', `quantity` = '" . (int)$data['quantity'] . "', `minimum` = '" . (int)$data['minimum'] . "', `subtract` = '" . (isset($data['subtract']) ? (bool)$data['subtract'] : 0) . "', `stock_status_id` = '" . (int)$data['stock_status_id'] . "', `date_available` = '" . $this->db->escape((string)$data['date_available']) . "', `manufacturer_id` = '" . (int)$data['manufacturer_id'] . "', `shipping` = '" . (isset($data['shipping']) ? (bool)$data['shipping'] : 0) . "', `price` = '" . (float)$data['price'] . "', `points` = '" . (int)$data['points'] . "', `weight` = '" . (float)$data['weight'] . "', `weight_class_id` = '" . (int)$data['weight_class_id'] . "', `length` = '" . (float)$data['length'] . "', `width` = '" . (float)$data['width'] . "', `height` = '" . (float)$data['height'] . "', `length_class_id` = '" . (int)$data['length_class_id'] . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', `tax_class_id` = '" . (int)$data['tax_class_id'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `date_modified` = NOW() WHERE `product_id` = '" . (int)$product_id . "'");
		...
		//Change: Remove meta title, meta_description, tag from query to database. Fixes null given consignee_product deletion bug
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_description` SET `product_id` = '" . (int)$product_id . "', `language_id` = '" . (int)$language_id . "', `name` = '" . $this->db->escape($value['name']) . "', `description` = '" . $this->db->escape($value['description']) . "'");
		}
	...
	{
		$this->db->query("DELETE FROM `" . DB_PREFIX . "consignee_product` WHERE `product_id` = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_attribute` WHERE `product_id` = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_description` WHERE `product_id` = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_discount` WHERE `product_id` = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_filter` WHERE `product_id` = '" . (int)$product_id . "'");
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "consignee_product` SET `master_id` = '0' WHERE `master_id` = '" . (int)$product_id . "'");
	...
	public function addVariant(int $master_id, array $data): int
	{
		...
		if ($master_info) {
			// We use the override to override the master consignee_product values
			...
			// Options
			// product_option should not be used if variant consignee_product
			...
			// SEO
			// product_seo table is not overwritten because that needs to have unique seo keywords for every consignee_product
		...
		// consignee_product add with master consignee_product overridden values
	...
	public function editVariant(int $master_id, int $product_id, array $data): void
	{
		...
		// Override the variant consignee_product data with the master consignee_product values
	...
	{
		// product_option should not be passed to consignee_product variants
		unset($data['product_option']);

		// If consignee_product is master update variants
		$products = $this->model_catalog_product->getProducts(['filter_master_id' => $master_id]);

		foreach ($products as $consignee_product) {
			$product_data = [];

			// We need to convert JSON strings back into an array so they can be re-encoded to a string to go back into the database.
			$consignee_product['override'] = (array)json_decode($consignee_product['override'], true);
			$consignee_product['variant'] = (array)json_decode($consignee_product['variant'], true);

			// We use the override to override the master consignee_product values
			if ($consignee_product['override']) {
				$override = $consignee_product['override'];
			} else {
				$override = [];
			}
			...
			// Now we want to
			foreach ($consignee_product as $key => $value) {
				// So if key not in override or ignore list we replace with master value
			...
			// Descriptions
			$product_descriptions = $this->model_catalog_product->getDescriptions($consignee_product['product_id']);
			...
			// Attributes
			if (isset($override['product_attribute'])) {
				$product_data['product_attribute'] = $this->model_catalog_product->getAttributes($consignee_product['product_id']);
			}

			// Category
			if (isset($override['product_category'])) {
				$product_data['product_category'] = $this->model_catalog_product->getCategories($consignee_product['product_id']);
			}

			// Discounts
			if (isset($override['product_discount'])) {
				$product_data['product_discount'] = $this->model_catalog_product->getDiscounts($consignee_product['product_id']);
			}

			// Downloads
			if (isset($override['product_download'])) {
				$product_data['product_download'] = $this->model_catalog_product->getDownloads($consignee_product['product_id']);
			}

			// Filters
			if (isset($override['product_filter'])) {
				$product_data['product_filter'] = $this->model_catalog_product->getFilters($consignee_product['product_id']);
			}

			// Images
			if (isset($override['product_image'])) {
				$product_data['product_image'] = $this->model_catalog_product->getImages($consignee_product['product_id']);
			}

			// Layouts
			if (isset($override['product_layout'])) {
				$product_data['product_layout'] = $this->model_catalog_product->getLayouts($consignee_product['product_id']);
			}

			// Subscription
			if (isset($override['product_subscription'])) {
				$product_data['product_subscription'] = $this->model_catalog_product->getSubscriptions($consignee_product['product_id']);
			}

			// Related
			if (isset($override['product_related'])) {
				$product_data['product_related'] = $this->model_catalog_product->getRelated($consignee_product['product_id']);
			}

			// Rewards
			if (isset($override['product_reward'])) {
				$product_data['product_reward'] = $this->model_catalog_product->getRewards($consignee_product['product_id']);
			}

			// SEO
			$product_data['product_seo_url'] = $this->model_catalog_product->getSeoUrls($consignee_product['product_id']);

			// Specials
			if (isset($override['product_special'])) {
				$product_data['product_special'] = $this->model_catalog_product->getSpecials($consignee_product['product_id']);
			}

			// Stores
			if (isset($override['product_store'])) {
				$product_data['product_store'] = $this->model_catalog_product->getStores($consignee_product['product_id']);
			}
			...
			$this->model_catalog_product->editProduct($consignee_product['product_id'], $product_data);
		}
	}
	...
	{
		$this->db->query("UPDATE `" . DB_PREFIX . "consignee_product` SET `rating` = '" . (int)$rating . "', `date_modified` = NOW() WHERE `product_id` = '" . (int)$product_id . "'");
	}
	...
	{
		$query = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "consignee_product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id`) WHERE p.`product_id` = '" . (int)$product_id . "' AND pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'");
	...
	{
		//$sql = "SELECT * FROM `" . DB_PREFIX . "consignee_product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id`) WHERE pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
	...
	{
		$sql = "SELECT COUNT(DISTINCT p.`product_id`) AS `total` FROM `" . DB_PREFIX . "consignee_product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id`) WHERE pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
	...
	{
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee_product` WHERE `tax_class_id` = '" . (int)$tax_class_id . "'");
	...
	{
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee_product` WHERE `stock_status_id` = '" . (int)$stock_status_id . "'");
	...
	{
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee_product` WHERE `weight_class_id` = '" . (int)$weight_class_id . "'");
	...
	{
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee_product` WHERE `length_class_id` = '" . (int)$length_class_id . "'");
	...
	{
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee_product` WHERE `manufacturer_id` = '" . (int)$manufacturer_id . "'");
